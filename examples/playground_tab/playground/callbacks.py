...
from .optimizers import ALGO_ORDER, ALGO_PARAMS, run_optimization
...
def _cumulative_axis_points(pts: list, upto: int, dim: int, labels: list[str]):
    """Every coordinate sampled up to frame ``upto``, flattened to (x, axis) pairs."""
    mx, my = [], []
...
def _add_axis_overlay(fig: go.Figure, runs: list[dict], path_idx, frame: int,
                      dim: int, labels: list[str]) -> None:
    """Overlay the selected run's points-so-far and its current position."""
    if not runs or path_idx is None or not (0 <= path_idx < len(runs)):
        return
    ...
    mx, my = _cumulative_axis_points(pts, f, dim, labels)
    fig.add_trace(go.Scatter(
...
def _axis_strips_figure(map_data: dict, runs: list[dict], path_idx, frame: int,
    ...
    _add_axis_overlay(fig, runs, path_idx, frame, dim, labels)
...
def _register_control_visibility(app) -> None:
    """Show/hide the control rows that apply to the current algorithm and map mode."""
...
def _register_map_callbacks(app) -> None:
    """Regenerate the synthetic landscape, behind a confirm since it wipes runs."""
    ...
    def regen_map(_submit, mode, n_optima, spread, dim):
        seed = random.randrange(1_000_000)  # toy-landscape seed, not security
...
def _register_run_callbacks(app) -> None:
    """Run one algorithm or every algorithm on the current map, and clear runs."""
...
def _register_transport(app) -> None:
    """Playback transport: play / pause / step / replay / timer."""
...
def _register_draw(app) -> None:
    """Redraw the landscape, convergence curve and per-axis strips for a frame."""
...
def register_playground_callbacks(app) -> None:
    """Register every Playground-tab callback.

    This is the ``register`` hook of the tab's :class:`sofaopt.DashboardTab`;
    the groups below are split only to keep each registrar small.
    """
    _register_control_visibility(app)
    _register_map_callbacks(app)
    _register_run_callbacks(app)
    _register_transport(app)
    _register_draw(app)
